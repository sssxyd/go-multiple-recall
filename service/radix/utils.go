...
func pinyinOfWord(word string) (py string, py_abbr string) {
	// 汉字转拼音
	args := pinyin.NewArgs()
	...
	for _, r := range word {
		if unicode.Is(unicode.Han, r) { // 判断是否为汉字
			...
			if len(pinyinResult) > 0 && len(pinyinResult[0]) > 0 {
				...
				py_abbr += string(pinyinResult[0][0][0]) // 取拼音首字毧
			}
		} else if r == '*' { // 掩码字符
			py += "* "
			py_abbr += "*"
	...
	return strings.TrimRight(py, " "), py_abbr
}
...
func calcTireWordLength(word string) int {
	len := 0
	for _, r := range word {
		if unicode.Is(unicode.Han, r) {
			len += 2
...
func TrieSplitWord(word string, outOfOrder bool, maskOffCode int) []string {
	...
	for i := 1; i < word_len-2; i++ {
		...
		if calcTireWordLength(sub_word) < 4 {
			break
		}
